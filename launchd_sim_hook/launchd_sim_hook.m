...
void jbserver_sim_get_launchd(mach_msg_header_t *message) {
    ...
    // Get rid of the temporary port used to send our port over
    mach_msg_destroy(&reply_msg->header);
    // xpc_receive_mach_msg will clean it up for us
    //mach_port_deallocate(mach_task_self(), reply_msg->header.msgh_remote_port);
}
